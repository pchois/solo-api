...
const retrieveModels = (req, res, next) => {
  db.all("SELECT model_id, name FROM Models m JOIN MakeModelMap mmm on m.id = mmm.model_id WHERE mmm.make_id = $id ORDER BY name ASC;",
  {$id: req.query.makeId},
  (err, models) => {
    if(err){
      res.status(500).send(err);
...
const retrieveTrims = (req, res, next) => {
  db.all("SELECT trim_id, name from Trims t JOIN ModelTrimMap mtm on t.id = mtm.trim_id WHERE mtm.make_id = $make_id AND mtm.model_id = $model_id ORDER BY name ASC;",
...
const retrieveYears = (req, res, next) => {
  db.all("SELECT year_id, name FROM Years y JOIN TrimYearMap tym ON y.id = tym.year_id WHERE tym.make_id = $make_id AND tym.model_id = $model_id AND tym.trim_id = $trim_id ORDER BY name ASC;",
...
const retrieveYearsS3 = (req, res, next) => {
  db.all("SELECT year_id, name FROM Years y JOIN ModelYearMap mym ON y.id = mym.year_id WHERE mym.make_id = $make_id AND mym.model_id = $model_id ORDER BY name ASC;",
  {$make_id: req.query.makeId, $model_id: req.query.modelId},
  (err, yearss3) => {
    if(err){
      res.status(500).send(err);
...
const retrieveClassS1 = (req, res, next) => {
  db.all("SELECT class_id, name FROM Classes c JOIN Scenario1 s1 ON c.id = s1.class_id WHERE s1.make_id = $make_id AND s1.model_id = $model_id ORDER BY name ASC;",
  {$make_id: req.query.makeId, $model_id: req.query.modelId},
  (err, classes1) => {
    if(err){
      res.status(500).send(err);
...
const retrieveClassS2 = (req, res, next) => {
  db.all("SELECT class_id, name FROM Classes c JOIN Scenario2 s2 ON c.id = s2.class_id WHERE s2.make_id = $make_id AND s2.model_id = $model_id AND s2.trim_id = $trim_id ORDER BY name ASC;",
  {$make_id: req.query.makeId, $model_id: req.query.modelId, $trim_id: req.query.trimId},
  (err, classes2) => {
    if(err){
      res.status(500).send(err);
...
const retrieveClassS3 = (req, res, next) => {
  db.all("SELECT class_id, name FROM Classes c JOIN Scenario3 s3 ON c.id = s3.class_id WHERE s3.make_id = $make_id AND s3.model_id = $model_id AND s3.year_id = $year_id ORDER BY name ASC;",
...
const retrieveClassS4 = (req, res, next) => {
  db.all("SELECT class_id, name FROM Classes c JOIN Scenario4 s4 ON c.id = s4.class_id WHERE s4.make_id = $make_id AND s4.model_id = $model_id AND s4.trim_id = $trim_id AND s4.year_id = $year_id ORDER BY name ASC;",
